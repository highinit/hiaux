...
template <class KeyType, class ShardInfo>
class hSharder
{
protected:
    ...
    void setShardsCount(int v, int r)
    {
        this->nvshards = v;
        this->nrshards = r;
    }
...
public:
    ...
    ShardInfo getRealShard(const KeyType &key)
    {
        int hash =  crc32( (const unsigned char *) key.data(), key.length());
        hash = hash ? (hash>=0) : -hash;
        return virtual_shards[ hash % nvshards ];
    }
